...
public final class GrantSubcommand implements Subcommand {
    @Override
    ...
    public void execute(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (type == null) {
            sender.sendMessage(CLI.failed.append(Language.translate("command.grant.not_found", sender, "type=" + args[1])));
            return;
        }

        if (type.isGranted(player)) {
            sender.sendMessage(CLI.warn.append(Language.translate("command.grant.already", sender, "player=" + args[0], "type=" + args[1])));
            return;
        }

        type.grant(player);
        sender.sendMessage(CLI.successful.append(Language.translate("command.grant.complete", sender, "player=" + args[0], "type=" + args[1])));
    }
}
